...
class Leveler(commands.Cog):
    ...
    async def top(self, ctx, *options):
        ...
        Add `-global` parameter for global or `-rep` for reputation.
        """
        server = ctx.guild
        user = ctx.author
    ...
    async def profilecolors(self, ctx, section: str, color: str):
        ...
        For color, you can indicate : `default`, `white`, `HEX code` (#000000) or `auto`.
    ...
    async def rankcolors(self, ctx, section: str, color: str = None):
        ...
        Section can be : `exp`, `info` or `all`.
    ...
    async def _auto_color(self, ctx, url: str, ranks):
        if "scipy" not in locals():
            await ctx.send("**Missing a required package. Cannot use Autocolor feature.**")
            return
        phrases = ["Calculating colors...", "Generating colors..."]  # in case I want more
    ...
    @commands.guild_only()
    async def linkrole(
        self, ctx, role: discord.Role, level: int, remove_role: discord.Role = None
    ):
        """Associate a role with a level.

        If role contain space, use double quote.
        You can remove role if you give one.
        """
        server = ctx.guild
        ...
        if role.managed is True:
            await ctx.send(
                "I won't be able to give a role from an integration. (Bots role, Boosters, etc...)"
            )
            return
        if not server_roles:
            new_server = {
                ...
                "roles": {
                    str(role.id): {
                        "level": str(level),
                        "remove_role": str(remove_role.id) if remove_role else None,
                    }
                },
            }
            await db.roles.insert_one(new_server)
        else:
            if str(role.id) not in server_roles["roles"]:
                server_roles["roles"][str(role.id)] = {}

            server_roles["roles"][str(role.id)]["level"] = str(level)
            server_roles["roles"][str(role.id)]["remove_role"] = (
        ...
        if remove_role is None:
            ...
            await ctx.send(
                ...
                "Will also remove `{}` role.**".format(role.name, level, remove_role)
            )

    @checks.mod_or_permissions(manage_roles=True)
    @role.command(name="unlink")
    @commands.guild_only()
    async def unlinkrole(self, ctx, *, role: discord.Role):
        """Delete a role/level association."""
        server = ctx.guild

        server_roles = await db.roles.find_one({"server_id": str(server.id)})
        if not server_roles:
            await ctx.send("There are no roles to be deleted.")
            return
        ...
        if str(role.id) in roles:
            await ctx.send(
                "**Role/Level association `{}`/`{}` removed.**".format(
                    role.name, roles[str(role.id)]["level"]
                )
            )
            ...
            await ctx.send(
                "**The `{}` role is not linked to any levels!**".format(role.name)
            )
    ...
    async def listrole(self, ctx):
        ...
        if server_roles is None or not server_roles.get("roles"):
            ...
            msg = "**Role** → Level\n"
            for role in roles:
                role_obj = server.get_role(int(role))
                if roles[role]["remove_role"]:
                    role_remove_obj = server.get_role(int(roles[role]["remove_role"]))
                    msg += "**• {} →** {} (Removes: {})\n".format(
                        role_obj.name, roles[role]["level"], role_remove_obj.name
                    )
                else:
                    msg += "**• {} →** {}\n".format(role_obj.name, roles[role]["level"])
    ...
    async def delprofilebg(self, ctx, name: str):
        ...
        Indicate the background's name.
        """
    ...
    #@checks.is_owner()
    #@commands.command() #3640
    #async def updateroles(self, ctx):
        #"""Update roles database."""
        #await ctx.send(str(sr))
        #await ctx.send("**WARNING** This will update the whole role's table. This can take a while.")
        #servers = self.bot.guilds
        #for server in servers:
            #await rewrite_levels(self, server.id)
            #await ctx.send("**Role(s) rewrite made for server {}**".format(server.name))
    ...
    async def _handle_levelup(self, user, userinfo, server, channel):
        ...
        try:
            ...
            if server_roles is not None:
                for role in server_roles["roles"].keys():
                    if int(server_roles["roles"][role]["level"]) == int(new_level):
                        add_role = server.get_role(int(role))
                        ...
                        remove_role_id = server_roles["roles"][role]["remove_role"]
                        if remove_role_id is not None:
                            remove_role = server.get_role(int(remove_role_id))
                            if remove_role is not None:
                                try:
                                    await user.remove_roles(remove_role, reason="Levelup")
                                    await channel.send(f"Removed {remove_role.name}")
                                except discord.Forbidden:
                                    await channel.send(
            ...
            await channel.send(f"Error. Role was not given: {exc}")
        try:
            server_linked_badges = await db.badgelinks.find_one(
                {"server_id": str(server.id)}
            )
    ...
    @commands.command()
    @checks.is_owner()
    async def rewritetest(self, ctx): # For testing purpose, need to be run at loading.
        server_id = ctx.guild.id
        server = self.bot.get_guild(server_id)
        db_droles = await db.roles.find_one({"server_id": str(server_id)})
        if db_droles is None:
            print("Nothing to change, aborting.")
            return
        db_roles = db_droles['roles']
        await ctx.send(str(db_roles))
        for role in db_roles.keys():
            try:
                role_obj = discord.utils.find(lambda r: r.name == role, server.roles)
            except Exception as exc:
                await ctx.send("Role cannot be found. Aborting.")